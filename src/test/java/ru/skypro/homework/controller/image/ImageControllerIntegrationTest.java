...
public class ImageControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    void getAvatar_ShouldReturnImage_WhenExists() {
        ResponseEntity<byte[]> response =
                restTemplate.getForEntity(baseUrl() + "/avatars/avatar.jpg", byte[].class);
    ...
    void getAvatar_ShouldReturn404_WhenNotExists() {
        ResponseEntity<byte[]> response =
                restTemplate.getForEntity(baseUrl() + "/avatars/nonexistent.jpg", byte[].class);
    ...
    void getAdImage_ShouldReturnImage_WhenExists() {
        ResponseEntity<byte[]> response =
                restTemplate.getForEntity(baseUrl() + "/ads-images/ad.jpg", byte[].class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    }
    ...
    void getAdImage_ShouldReturn404_WhenNotExists() {
        ResponseEntity<byte[]> response =
                restTemplate.getForEntity(baseUrl() + "/ads-images/nonexistent.jpg", byte[].class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
    }
}
